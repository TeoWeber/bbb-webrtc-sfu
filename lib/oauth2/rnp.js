...
const EMBED_URL = "/portal/embed-transmission-not-visible";
...
const getTransmissionEmbedUrl = (params) => {
  return 'https://' + HOSTNAME + EMBED_URL + "?" + querystring.stringify(params);
}
...
const getStreamKey = (auth, callback) => {
  ...
  findOrCreateTransmission(auth, (err, transmission) => {
    ...
    return callback(null, transmission.rtmp, getTransmissionEmbedUrl({idItem: transmission.id}));
  });
}
